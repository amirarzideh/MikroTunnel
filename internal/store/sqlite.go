...
`)
	...
	if err := s.ensureColumn(ctx, "tunnels", "peer_address", "TEXT NOT NULL DEFAULT ''"); err != nil { return err }
...
func (s *SQLiteStore) CreateTunnel(ctx context.Context, t domain.Tunnel) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO tunnels (id,name,type,local_endpoint,remote_endpoint,address,peer_address,mtu,ttl,description,masquerade,desired_state,actual_state,last_error,failure_count,retry_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`, t.ID, t.Name, t.Type, t.Local, t.Remote, t.Address, t.PeerAddress, t.MTU, t.TTL, t.Description, t.Masquerade, t.DesiredState, t.ActualState, t.LastError, t.FailureCount, nullableTime(t.RetryAt), t.CreatedAt.UTC().Format(time.RFC3339Nano), t.UpdatedAt.UTC().Format(time.RFC3339Nano))
	return err
}

func (s *SQLiteStore) UpdateTunnel(ctx context.Context, t domain.Tunnel) error {
	res, err := s.db.ExecContext(ctx, `UPDATE tunnels SET name=?, type=?, local_endpoint=?, remote_endpoint=?, address=?, peer_address=?, mtu=?, ttl=?, description=?, masquerade=?, desired_state=?, actual_state=?, last_error=?, failure_count=?, retry_at=?, updated_at=? WHERE id=?`, t.Name, t.Type, t.Local, t.Remote, t.Address, t.PeerAddress, t.MTU, t.TTL, t.Description, t.Masquerade, t.DesiredState, t.ActualState, t.LastError, t.FailureCount, nullableTime(t.RetryAt), t.UpdatedAt.UTC().Format(time.RFC3339Nano), t.ID)
	if err != nil {
		return err
	}
...
func (s *SQLiteStore) GetTunnel(ctx context.Context, id string) (domain.Tunnel, error) {
	row := s.db.QueryRowContext(ctx, `SELECT id,name,type,local_endpoint,remote_endpoint,address,peer_address,mtu,ttl,description,masquerade,desired_state,actual_state,last_error,failure_count,retry_at,created_at,updated_at FROM tunnels WHERE id=?`, id)
	return scanTunnel(row)
}
func (s *SQLiteStore) ListTunnels(ctx context.Context) ([]domain.Tunnel, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id,name,type,local_endpoint,remote_endpoint,address,peer_address,mtu,ttl,description,masquerade,desired_state,actual_state,last_error,failure_count,retry_at,created_at,updated_at FROM tunnels ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func scanTunnel(row scanner) (domain.Tunnel, error) {
	...
	err := row.Scan(&t.ID, &t.Name, &t.Type, &t.Local, &t.Remote, &t.Address, &t.PeerAddress, &t.MTU, &t.TTL, &t.Description, &t.Masquerade, &t.DesiredState, &t.ActualState, &t.LastError, &t.FailureCount, &retry, &created, &updated)
