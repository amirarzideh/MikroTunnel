(() => {
  ...
  async function load() {
    ...
    try {
      ...
      $('tunnels').innerHTML = tunnels.items.length ? tunnels.items.map(t => `<article class="tunnel"><div><div class="tunnel-name">${safe(t.name)}</div><span class="tunnel-detail">${safe(t.local_endpoint)} to ${safe(t.remote_endpoint)} · ${safe(t.address)}</span></div><div class="tunnel-actions">${badge(t.actual_state)} ${badge(t.desired_state)}<button class="action" data-id="${safe(t.id)}" data-action="${t.desired_state === 'enabled' ? 'disable' : 'enable'}">${t.desired_state === 'enabled' ? 'Disable' : 'Enable'}</button><button class="action danger" data-id="${safe(t.id)}" data-action="delete">Remove</button></div></article>`).join('') : '<p class="empty">No tunnels have been configured.</p>';
